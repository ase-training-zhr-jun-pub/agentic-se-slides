---
layout: chapter
no: 4
background: /backgrounds/4.webp
---

# Accelerating Implementation

<!-- Master reference: Chapter 4 / Slide 141 -->

---
src: ./context-challenges/slides.md
---

---
src: ./additional-agent-features/slides.md
---

---
layout: intro
background: apricot
---

# Development Processes

<!-- Master reference: Chapter 4 / Slide 167 -->

---
src: ./vibe-coding/slides.md
---

---
layout: exercise
chapter: 4
exercise: 4
task: Vibe Coding
command: git merge uebung-3-4
---

<!-- Master reference: Chapter 4 / Slide 170 -->

---
src: ./spec-driven-development/slides.md
---

---
src: ./coding-with-agents/slides.md
---

---
layout: exercise
chapter: 4
exercise: 6
task: Coding Buddy
command: git merge uebung-3-6
---

<!-- Master reference: Chapter 4 / Slide 186 -->

---
layout: exercise
chapter: 4
exercise: 7
task: Create a pull request
command: git merge uebung-3-7
---

<!-- Master reference: Chapter 4 / Slide 187 -->

---
layout: exercise
chapter: 4
exercise: 8
task: Making Reviews
command: git merge uebung-3-8
---

<!-- Master reference: Chapter 4 / Slide 188 -->

---
src: ./development-use-cases/slides.md
---

---
layout: exercise
chapter: 4
exercise: 9
task: Optimize context with subagents
command: git merge uebung-3-9
---

<!-- Master reference: Chapter 4 / Slide 206 -->

---
layout: center
---

<img src="./development-use-cases/assets/programmer-excuse-generating.png" class="mx-auto max-h-[78%] w-auto object-contain" />

<!-- Master reference: Chapter 4 / Slide 207 -->

---
layout: exercise
chapter: 4
exercise: 10
task: Parallelize work
command: git merge uebung-3-10
---

<!-- Master reference: Chapter 4 / Slide 208 -->

---
layout: center
background: petrol
---

## Chapter 4 -- Key Takeaways

- Curated context enables bigger tasks
- Use TDD to enhance the feedback loop
- You push it, you own it
- No vibe coding for production

<!-- Master reference: Chapter 4 / Slide 209 -->
